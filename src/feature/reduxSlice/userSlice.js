...
export const checkPasswordUser = createAsyncThunk(
  "user/checkPasswordUser",
  async (value, { rejectWithValue }) => {
    try {
      const res = await api.checkPasswordUser(value);
      return res.data;
    } catch (error) {
      return rejectWithValue(error.response.data);
    }
...
const userSlice = createSlice({
  ...
  extraReducers: {
    ...
    [checkPasswordUser.pending]: (state) => {
      state.isLoadingCheckPassword = true;
      state.errorCheckPassword = "";
    },
    [checkPasswordUser.fulfilled]: (state, action) => {
      state.isLoadingCheckPassword = false;
      state.isPasswordTrue = action.payload?.oldPassword;
    },
    [checkPasswordUser.rejected]: (state, action) => {
      state.isLoadingCheckPassword = false;
